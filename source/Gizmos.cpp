...
Gizmos::Gizmos(unsigned int a_maxLines, unsigned int a_maxTris)
	...
	m_points(new GizmoPoint[m_maxPoints]),
	m_transparentTris(new GizmoTri[a_maxTris])
{
	...
	const char* szInput[] = { "Position", "Colour" };
	const char* szOutput[] = { "FragColor" };
...
#pragma region line/tri
	...
	glBufferData(GL_ARRAY_BUFFER, m_maxTris * sizeof(GizmoTri), m_transparentTris, GL_STATIC_DRAW);
	...
	//Add a new vertex attribute pointer for the uv coords
	//glVertexAttribPointer(2, 4, GL_FLOAT, GL_FALSE, sizeof(GizmoVertex), ((char*)0) + 32);
...
#pragma endregion
	...
	glBufferData(GL_ARRAY_BUFFER, m_maxPoints * sizeof(GizmoPoint), m_points, GL_STATIC_DRAW);

	glEnableVertexAttribArray(0);
	glEnableVertexAttribArray(1);
	glVertexAttribPointer(0, 4, GL_FLOAT, GL_FALSE, sizeof(GizmoPoint), 0);
	glVertexAttribPointer(1, 4, GL_FLOAT, GL_TRUE, sizeof(GizmoPoint), ((char*)0) + 16);
	...
	glBufferData(GL_ELEMENT_ARRAY_BUFFER, m_maxPoints * sizeof(unsigned int), nullptr, GL_STATIC_DRAW);
...
//Bind a texture to render on to the particles
void Gizmos::addTexture()
{
	//Create a texture manager or return the instanced
	TextureManager* textureManager = TextureManager::CreateInstance();

	//Load a texture from the bin folder
	unsigned int texture = textureManager->LoadTexture("./images/1.jpg");
...
void Gizmos::addBillBoard(const glm::vec4& a_position, const glm::vec4& nVx, const glm::vec2& a_size, glm::vec4& a_colour)
{
	glm::vec3 vVerts[4];
	glm::vec3 vY(0, a_size.y, 0);

	vVerts[0] = a_position.xyz - nVx.xyz - vY;
	vVerts[1] = a_position.xyz - nVx.xyz + vY;
	vVerts[2] = a_position.xyz + nVx.xyz + vY;
	vVerts[3] = a_position.xyz + nVx.xyz - vY;

	addTri(vVerts[0], vVerts[2], vVerts[1], a_colour);
	addTri(vVerts[2], vVerts[0], vVerts[3], a_colour);
}
...
//Add a point to the m_points array of GizmoPoint at the position of m_pointCount
void Gizmos::addPoint(const glm::vec4& a_p, const glm::vec4& a_colour)
{
	if (sm_singleton != nullptr)
	{
		sm_singleton->m_points[sm_singleton->m_pointCount].pos= a_p;
		sm_singleton->m_points[sm_singleton->m_pointCount].colour = a_colour;

		sm_singleton->m_pointCount++;
	}
}

void Gizmos::draw(const glm::mat4& a_projectionView, const glm::mat4& a_camera)
{
	if (sm_singleton != nullptr &&
		(sm_singleton->m_lineCount > 0 || sm_singleton->m_triCount > 0 || sm_singleton->m_transparentTriCount > 0))
	{
		...
		//Camera uniform
		unsigned int cameraViewUniform = glGetUniformLocation(sm_singleton->m_programID, "CameraView");
		glUniformMatrix4fv(cameraViewUniform, 1, false, glm::value_ptr(a_camera));
		...
		//If the m_pointCount is greater than 0
		if (sm_singleton->m_pointCount > 0)
		{
			//Bind a buffer
			glBindBuffer(GL_ARRAY_BUFFER, sm_singleton->m_pointVBO);
			glBufferSubData(GL_ARRAY_BUFFER, 0, sm_singleton->m_pointCount * sizeof(GizmoPoint), sm_singleton->m_points);
