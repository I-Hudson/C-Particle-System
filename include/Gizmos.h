...
//Geometry shader renders all particles in the same spot 
...
public:
	...
	//Draw a bill board 
	static void		addBillBoard(const glm::vec4& a_position, const glm::vec4& nVx, const glm::vec2& a_size, glm::vec4& a_colour);
	//Load in a texture 
	static void		addTexture();
	//Draw a quad from a vec4 point
	static void		addPoint(const glm::vec4& a_p, const glm::vec4& a_colour);

	// draws current Gizmo buffers, either using a combined (projection * view) matrix, or separate matrices
	static void		draw(const glm::mat4& a_projectionView, const glm::mat4& a_camera);
	static void		draw(const glm::mat4& a_view, const glm::mat4& a_projection, const glm::mat4& a_camera);
...
private:
	...
	// Point data
	unsigned int	m_maxPoints = 32768;
...
inline void Gizmos::draw(const glm::mat4& a_view, const glm::mat4& a_projection, const glm::mat4& a_camera)
{
	draw(a_projection * a_view, a_camera);
}
